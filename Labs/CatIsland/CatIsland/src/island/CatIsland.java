...
public class CatIsland {
    ...
    public static void solveIsland3(Cat cat) throws CatInWaterException {
        // WRITE YOUR CODE HERE
        // moves: left, up, down, down, left, right, up, right, right, right, up, down, down
        cat.moveLeft();
        cat.moveUp();
        cat.moveDown();
        cat.moveDown();
    ...
    public static void solveIsland4(Island islandFour) throws CatInWaterException {
        ...
        Cat cat = new Cat("Cat", islandFour, 2, 2, island.constants.Color.ORANGE);
    ...
    public static void solveIsland5Recursive(Cat cat, Tile[][] tiles, Tile prev) throws CatInWaterException {
        ...
        if (col+1 < tiles[0].length && tiles[row][col+1].type == Tile.LAND && tiles[row][col+1] != prev) {
            cat.moveRight();
            solveIsland5Recursive(cat, tiles, tiles[row][col]);
        } else if (row+1 < tiles.length && tiles[row+1][col].type == Tile.LAND && tiles[row+1][col] != prev) {
            cat.moveDown();
            solveIsland5Recursive(cat, tiles, tiles[row][col]);
        } else if (col-1 >= 0 && tiles[row][col-1].type == Tile.LAND && tiles[row][col-1] != prev) {
            cat.moveLeft();
            solveIsland5Recursive(cat, tiles, tiles[row][col]);
        } else if (row-1 >= 0 && tiles[row-1][col].type == Tile.LAND && tiles[row-1][col] != prev) {
            cat.moveUp();
            solveIsland5Recursive(cat, tiles, tiles[row][col]);
